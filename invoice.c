...
void cost_wholesale(struct scrllst_msg_rec *msg)
{
...
void cost_retail(struct scrllst_msg_rec *msg)
{
...
void retail_wholesale(struct scrllst_msg_rec *msg)
{
...
void wholesale_only(struct scrllst_msg_rec *msg)
{
	sprintf (vheading, "%s%s", VHEADING, "Cost      |W/Sale");
...
void retail_only(struct scrllst_msg_rec *msg)
{
	sprintf (vheading, "%s%s", VHEADING, "Cost      |Retail");
...
void quotesrch (msg,filedef)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
{
   numbers_search(msg,filedef,0,2);
}
void invsrch (msg,filedef)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
{
   numbers_search(msg,filedef,0,3);
}
void jobsrch (msg,filedef)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
{
...
void assign_job_no(msg,filedef)
...
struct filestruct *filedef;
{
   ...
   if(rmstest(rms_status,1,RMS$_OK_RLK)){
       warn_user("Write Locked Master  %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
       return;
   }
   ...
   if (quote_buff.job_no == 0.0) {
      ...
      if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_DUP,RMS$_OK_DUP)) {
          warn_user("UPD$ write fail- rms retcode: %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
          return;
      }  
   } else sys$free(&quote_file.rab);
}
...
void assign_invoice_no(msg,filedef)
struct scrllst_msg_rec *msg;
struct filestruct *filedef;
{
...
int	check_tax(char *test)
{
	*test = toupper(*test);
	...
	case 'X' :
		...
		return(TRUE);
	default :
		warn_user("Must Be   A,B,C,D,E or X %s %u",filterfn(__FILE__), __LINE__);
		return(FALSE);
	}
}
...
int quote_where_joics_eq()
{
   struct quote_rec *dtr = (struct quote_rec *)quote_file.fileio;

   if( dtr->cust_no == fileio.record.joic_no) return(TRUE);
    else return(FALSE);
}
...
void disassemble(char *,struct filestruct *,int,struct line_rec **,uint16);
...
int quote_unload_editor_buffer(curs,filedef)
...
struct filestruct *filedef;
{
   ...
   lines = assemble(&filedef->fileio,filedef,sizeof(struct quote_part_rec),&curs);

   dtr->reclen = sizeof(quote_buff) + (lines * sizeof(struct quote_part_rec));

   return(dtr->reclen);
}

void quote_load_editor_buffer (ptr,filedef)
struct line_rec **ptr;
struct filestruct *filedef;
{
   struct quote_rec *dtr = (struct quote_rec *)filedef->fileio;
  
   disassemble(filedef->fileio,filedef,sizeof(struct quote_part_rec),ptr,dtr->reclen);
   while((*ptr) != NULL && (*ptr)->last != NULL) (*ptr) = (*ptr)->last;


}

void numbers_search(msg,filedef,frstchr,keynum)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
char frstchr;
int keynum;
	...
	if (wch != SMG$K_TRM_CTRLZ) {  /*scan code for ESC  */
      ...
      if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_RNF,RMS$_OK_RLK)) { 
         warn_user("$GET %s %s %s %u",rmslookup(rms_status),
                           filterfn(quote_file.filename),filterfn(__FILE__),__LINE__);
         return;
      }

      if (rms_status == RMS$_RNF) 
         warn_user("$FIND Record Not Found %s %u",filterfn(__FILE__),__LINE__);
      else {
...
void quote_search (msg,filedef,frstchr)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
char frstchr;
{
...
void fork_quote_file(msg,filedef)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
{
   char	test;
   ...
   sprintf (vheading, "%s%s", VHEADING, 
             (test == 'A' || test == 'W') ? "Cost      |W/Sale" : (test == 'Y') ? "Retail    |W/Sale"
	     : "Cost      |Retail");
...
void fork_all_invoices(void)
{
	...
	test = master_master.d_flag_w;
	sprintf (vheading, "%s%s", VHEADING, (test == 'A' || test == 'W') ? "Cost      |W/Sale" : (test == 'Y') ? "Retail    |W/Sale"
	     : "Cost      |Retail");
