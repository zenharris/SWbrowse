...
int select_inventory_item(struct kit_part_rec *crnt_line)
{
   int ret,status;
...
int get_inventory_item(char *code)
{
   int lnth,status;
...
int kitsed(edtstr,row,col,lnth,attr,pos,control)
char *edtstr;
...
unsigned int *control;
{
   ...
   if (*control & INIT_ONLY) {
      for (i = 0; i < LINEDLEN;i++) kit_line[i].row = row;
      init_screen(kit_line,LINEDLEN);
      return(TRUE);
   ...
   for (i = 0; i < LINEDLEN;i++) kit_line[i].row = row;
   init_screen(kit_line,LINEDLEN);
   ret = enter_screen(kit_line,LINEDLEN,&edited,breakout,G_filedef,&strtfld);
   switch (ret) {
      ...
      case SMG$K_TRM_HT: strtfld = 0;
                        break;
      case SMG$K_TRM_UP: curs = G_filedef->entry_screen[G_filedef->entry_comment].field;
