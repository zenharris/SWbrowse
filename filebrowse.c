...
void get_time(struct timebuff *result)
{
   int status;
...
char *print_time(struct timebuff *rawtime)
{
   int status;
   ...
   status = sys$asctim(0,
                       &atime,
                       rawtime, /*&quote_buff.invoice_date,*/
                       0);
   if (!$VMS_STATUS_SUCCESS (status)){ 
      warn_user("Time Error %s %u",filterfn(__FILE__),__LINE__);
      return("");
...
char *print_date(struct timebuff *rawtime)
{
   int status;
   static char timestr[SIZE_TIMESTR];
   $DESCRIPTOR(atime,timestr);
...
struct timebuff *dttodo (struct timebuff *date_time)
{
   char outstr[25] = "";
...
char *rmslookup(unsigned int rmscode)
{
   FILE *fptr;
   ...
   while (fscanf(fptr,"%s %u %s %s %u",dummy,&dummyint,dummy,fndate,&level) != EOF) {
      if (level == rmscode){
         fclose(fptr);
...
void unpad(char *source,int lnth)
{
   int i;
...
void spawn_scroll_window(filedef,lnth,width)
...
void spawn_edit_window(filedef,lnth,width)
struct filestruct *filedef;
int lnth,width;
{
   long x,y;
...
struct filestruct *filedef;
{
   ...
   void InsertNewLine(Ptr,filedef)
      ...
      struct filestruct *filedef;
   {
     ...
     if ((*Ptr)->Next != NULL) {
         ...
         if((*Ptr)->Prev != NULL) {
            ...
            DelPtr = (*Ptr);
            (*Ptr) = filedef->CurrentPtr = filedef->FirstPtr = filedef->LastPtr = NULL;
         }
...
int ListRead (ListLength,includefirst,reverse,filedef,middleout)
...
int middleout;
{
   ...
   for(number_elements = 0;
       number_elements < ListLength;
       /* number_elements++*/) {
...
/* ZEN 6-JUN-2023 21:32:14 */                                   
         if(!rmstest(rms_status,4,RMS$_NORMAL,RMS$_RNF,RMS$_OK_RRL,RMS$_OK_RLK)) 
            warn_user("GET$ %s %s %s %u",rmslookup(rms_status),filedef->filename,filterfn(__FILE__),__LINE__);
...
/*ZEN test*/ filedef->rab.rab$l_rop = RAB$M_NLK;
      ...
      if (!rmstest(rms_status,5,RMS$_NORMAL,RMS$_EOF,RMS$_OK_RRL,RMS$_RNF,RMS$_OK_RLK)) 
         warn_user("$GET %s %s %s %u",rmslookup(rms_status),filterfn(filedef->filename),filterfn(__FILE__),__LINE__); 
      else
      ...
      if (filedef->where!=NULL && (!(*filedef->where)())) return number_elements;
...
if (reverse)  
         ...
         if (filedef->setprompt != NULL) (*filedef->setprompt)(tempptr,filedef);
         else warn_user("Set List Prompt Not Set %s %u",filterfn(__FILE__),__LINE__);
...
struct filestruct *filedef;
{
   ...
   if (!rmstest(rms_status,6,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_OK_RLK,RMS$_EOF,RMS$_DEL)) 
      warn_user("$GET %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
   else
      if (rmstest(rms_status,3,RMS$_RNF,RMS$_RLK,RMS$_DEL))
         warn_user ("%s %s %u",rmslookup (rms_status),filterfn(__FILE__),__LINE__);
/*
         switch (rms_status) {
...
void ReadMiddleOut(Rec,filedef)
struct rfabuff *Rec;
struct filestruct *filedef;
{
...
*/
   ...
   while (rms_status == RMS$_DEL) {
      if (filedef->CurrentPtr == NULL) warn_user("CurrentPtr is NULL");
      ...
      if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_OK_RLK,RMS$_DEL)) {
         warn_user("Secondary fail %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
         exit(rms_status);
      }
   }     
 
         if(filedef->setlistkey != NULL) (*filedef->setlistkey)();
         else warn_user("Set List Key Not Set %s %u",filterfn(__FILE__),__LINE__);
...
*/
         ...
         testline = ListRead(filedef->CurrentLine+1 /*RegionLength/2*/,INCLUDEFIRST,TRUE,filedef,TRUE);
         if (testline < filedef->CurrentLine) {
            filedef->CurrentLine = testline;
            if (filedef->CurrentLine > 0) filedef->CurrentLine--;
         }
         ...
         if (!rmstest(rms_status,4,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_OK_RRL)) 
           {
            ...
            if (rmstest(rms_status,2,RMS$_RNF,RMS$_RLK))
             /*  if (rms_status == RMS$_RLK) warn_user ("%s %s %u",strerror (EVMSERR, rms_status),filterfn(__FILE__),__LINE__);
               else */ warn_user("%s %s %u",rmslookup(rms_status) ,filterfn(__FILE__),__LINE__); 
            else { 
                filedef->rab.rab$b_rac = RAB$C_SEQ;

                ListRead(filedef->BottomLine-(filedef->CurrentLine+filedef->TopLine),FALSE,FALSE,filedef,TRUE);

                if(filedef->CurrentPtr == NULL) filedef->CurrentPtr = filedef->FirstPtr;
...
int ReadNextBlock (Count,Rec,reverse,filedef)
int Count;
struct rfabuff *Rec;
int reverse;
struct filestruct *filedef;
{
   ...
   if (!rmstest(rms_status,5,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_OK_RLK,RMS$_DEL)) 
      warn_user("$GET %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
   else
      if (rmstest(rms_status,2,RMS$_RNF,RMS$_RLK))
         warn_user ("%s %s %u",rmslookup (rms_status),filterfn(__FILE__),__LINE__);
...
*/
   ...
   while (rms_status == RMS$_DEL) {
      if (filedef->CurrentPtr == NULL) warn_user("Current Ptr is NULL");

      if (reverse) {
         if (filedef->CurrentPtr == filedef->FirstPtr) bump = TRUE;
         ...
         if (filedef->CurrentPtr == filedef->LastPtr) bump = TRUE;
         DeleteLine(&filedef->LastPtr,filedef);
         if(bump) {
            filedef->CurrentPtr = filedef->LastPtr;
            /* ZEN Delete Last Line dilema */
            if(filedef->CurrentPtr != NULL && filedef->CurrentPtr->Next == NULL && filedef->CurrentLine  > 0)
               filedef->CurrentLine--;
            bump = FALSE;
         }
      }
      ...
      if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_OK_RLK,RMS$_DEL)) {
         ...
         filedef->rab.rab$b_krf = (reverse)?filedef->revrkeyid:filedef->fwrdkeyid;
         ...
         if (!rmstest(rms_status,5,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_OK_RLK,RMS$_OK_ALK)) 
            warn_user("$GET %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
         else
            if (rmstest(rms_status,2,RMS$_RNF,RMS$_RLK))
            /*   if (rms_status == RMS$_RLK) warn_user ("%s %s %u",strerror (EVMSERR, rms_status),filterfn(__FILE__),__LINE__);
               ...
               for(number_elements = 0;number_elements < Count ;/* number_elements++ */) {
                  ...
                  if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_EOF,RMS$_OK_RLK)) { 
                     warn_user("$GET %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
                     break; 
                  } else
                  ...
                  if (filedef->where!=NULL && !(*filedef->where)()) break; 
...
if (reverse) {
                     ...
                     if(filedef->ListCount > filedef->RegionLength*2)
                        if(filedef->FirstPtr != NULL && filedef->FirstPtr->Next != NULL) {
                           delptr = filedef->FirstPtr;
                           filedef->FirstPtr = filedef->FirstPtr->Next;
...
void Read_Directory(index_key,filedef)
int index_key;
struct filestruct *filedef;
{
   int number_elements;
   ...
   if(filedef->where == NULL) { 
      ...
      if (rms_status != RMS$_NORMAL) 
         warn_user("REWIND  Error %s rmsret: %u %s %u",filedef->filename,rms_status,filterfn(__FILE__),__LINE__); 
   }
...
*/
   ...
   void Increment (IncLine,filedef)
   ...
   void Decrement (DecLine,filedef)
...
*/
            ...
            LineNum++;
            if (LineNum+ filedef->TopLine > filedef->BottomLine) break;

            if (!trip && curs2->Next == NULL) {
               trip = TRUE;
               ReadNextBlock (filedef->RegionLength,&curs2->rfa,FALSE,filedef);
            }
...
struct timebuff time;
{
   ...
   int ReadSearchTerm (initial_value)
      char *initial_value;
   {
...
/*moved below      delete_editor_buffer(&filedef->entry_screen[filedef->entry_comment].field); */
   ...
   if(filedef->padrecord != NULL) (*filedef->padrecord)();
   ...
   if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_DUP,RMS$_OK_DUP)) {
      if(rms_status = RMS$_CHG)
         warn_user("Changed Fixed Key Error %s %u",filterfn(__FILE__),__LINE__);
      else
         warn_user("PUT/UPD write fail- rms retcode: %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); /*,filedef); */
      return(FALSE);
   } else
...
/*moved*/   delete_editor_buffer(&filedef->entry_screen[filedef->entry_comment].field);
         ...
         warn_user ("Existing record with same key. not written. %s %u",filterfn(__FILE__),__LINE__);
         return(FALSE);
      }                         
}
   ...
   void Edit_Current_Record (update,filedef)
   int update;
   struct filestruct *filedef;
   {
      ...
      int strtfld = 0;
         ...
         if (filedef->entry_comment != 0) {
            delete_editor_buffer(&filedef->entry_screen[filedef->entry_comment].field/*&start_line*/);
            if(filedef->loadvarbuff != NULL)
               (*filedef->loadvarbuff)(&filedef->entry_screen[filedef->entry_comment].field,filedef);
         }


         if(filedef->unpadrecord != NULL) (*filedef->unpadrecord)();

         if(update) {
            if(filedef->preedit != NULL) (*filedef->preedit)();
         } else {
            if(filedef->preinsert != NULL)
               if(!(*filedef->preinsert)()) return;
         }
...
/* ZEN order modification */
         ...
         if(filedef->recalc != NULL) (*filedef->recalc)();
         ...
         if (filedef->entry_comment != 0)
            delete_editor_buffer(&filedef->entry_screen[filedef->entry_comment].field);
...
if(!update){
                ...
                if(filedef->FirstPtr == NULL) {
                   ...
                   /* ZEN */ 
                   if (!file_saved) rfa_copy(&LFindRec,&filedef->CurrentPtr->rfa);
                   else  rfa_copy(&LFindRec,(struct rfabuff *)&filedef->rab.rab$w_rfa);
   ...
   struct filestruct *filedef;
   {
      ...
      if (!rmstest(rms_status,5,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_OK_RRL,RMS$_OK_RLK)) 
         warn_user("$GET %s %s %s %u",rmslookup(rms_status),filterfn(filedef->filename),filterfn(__FILE__),__LINE__); 
      else
         if (rmstest(rms_status,2,RMS$_RNF,RMS$_RLK))
         ...
         if(filedef->entry_comment != 0) {
            delete_editor_buffer(&filedef->entry_screen[filedef->entry_comment].field /*&start_line*/);
            if(filedef->loadvarbuff != NULL)
               (*filedef->loadvarbuff)(&filedef->entry_screen[filedef->entry_comment].field /*&start_line*/,
                                                                      filedef);
         }
...
/*ZEN */ if(filedef->unpadrecord != NULL) (*filedef->unpadrecord)();
...
/*ZEN */    if(filedef->unpadrecord != NULL) (*filedef->unpadrecord)();
   ...
   void Seek_and_Edit(Rec,update,filedef)
   struct rfabuff *Rec;
   int update;
   struct filestruct *filedef;
   {
   ...
   void Delete_Record(Rec,filedef)
   struct rfabuff *Rec;
   struct filestruct *filedef;
   {
      ...
      if (rms_status == RMS$_RNF) 
         warn_user("$DELETE %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
...
/* ZEN Delete Last Line dilema */
   ...
   void active_align_list(CPtr,FindRec,filedef)
   ...
   struct rfabuff *File_Browse(filedef,opmode)
   struct filestruct *filedef;
   enum filebrowse_mode opmode;
   {
      ...
      while (!exit) {
         Help("|^I Insert |^R Search |^E Edit   |^D Delete |^W Reindex| PgUp/PgDn  ^Z to exit");


         if (filedef->FirstPtr != NULL) {
            Redraw_Screen(filedef);
            clreol(crnt_window->display_id);
            HiLite(crnt_window->display_id,filedef->CurrentPtr->Prompt,filedef->CurrentLine+filedef->TopLine);
         } else Clear_Region(filedef->TopLine,filedef->BottomLine);
         Refresh(crnt_window->display_id);
...
/*            case SMG$K_TRM_PF4: */
            ...
            case SMG$K_TRM_DOWN:
               if (filedef->CurrentPtr->Next == NULL)
                  if (!ReadNextBlock(5,&filedef->LastPtr->rfa,FALSE,filedef)) break;
               LoLite(crnt_window->display_id,filedef->CurrentPtr->Prompt,filedef->CurrentLine+filedef->TopLine);
               Increment(&filedef->CurrentLine,filedef);
               filedef->CurrentPtr = filedef->CurrentPtr->Next;
               break;
            case SMG$K_TRM_UP:
               if (filedef->CurrentPtr->Prev == NULL)
                  if(!ReadNextBlock(5,&filedef->FirstPtr->rfa,TRUE,filedef)) break;
               LoLite(crnt_window->display_id,filedef->CurrentPtr->Prompt,filedef->CurrentLine+filedef->TopLine);
            ...
            case SMG$K_TRM_NEXT_SCREEN :
               for (i=0; i <=  filedef->BottomLine - filedef->TopLine; i++) {
                  if (filedef->CurrentPtr->Next == NULL)
                     if(!ReadNextBlock(filedef->RegionLength,&filedef->LastPtr->rfa,FALSE,filedef)) break;
                  filedef->CurrentPtr = filedef->CurrentPtr->Next;
               }
               break;
            case SMG$K_TRM_PREV_SCREEN:
               for (i= 0;i <= (filedef->BottomLine - filedef->TopLine)+filedef->CurrentLine; i++) {
                  if (filedef->CurrentPtr->Prev == NULL)
                     if(!ReadNextBlock(filedef->RegionLength,&filedef->FirstPtr->rfa,TRUE,filedef)) break;
                  filedef->CurrentPtr = filedef->CurrentPtr->Prev;
               }
               for (i=0;i< filedef->CurrentLine;i++) if(filedef->CurrentPtr->Next != NULL) 
                                   filedef->CurrentPtr = filedef->CurrentPtr->Next;
...
/* Reintroduce this block if ever needed   */
      ...
      if (!rmstest(rms_status,2,RMS$_NORMAL,RMS$_OK_RRL)) 
        warn_user("$GET %s %s %s %u",filterfn(filedef->filename),rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
...
void Remove_Record (msg,filedef)
struct linedt_msg_rec *msg; 
struct filestruct *filedef;
{
   if(filedef->CurrentPtr == NULL) return;
   ...
   if(strcmp(user_name,priv_user) != 0) {
      warn_user("Not Authorised to Delete %s %u",filterfn(__FILE__),__LINE__);
      return;
   }
