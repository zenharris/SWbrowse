...
check_code (char *test)
{
  ...
  if (!rmstest (rms_status, 3, RMS$_NORMAL, RMS$_RNF, RMS$_OK_RLK))
    {
      warn_user ("Error inventory file %s %s %u", rmslookup (rms_status),
  ...
  if (!rmstest (rms_status, 3, RMS$_NORMAL, RMS$_RNF, RMS$_OK_RLK))
    {
      warn_user ("Error kit file %s %s %u", rmslookup (rms_status),
                 filterfn (__FILE__), __LINE__);
      return (TRUE);
    }
  if (rmstest (rms_status, 2, RMS$_NORMAL, RMS$_OK_RLK))
  ...
  if (wch != SMG$K_TRM_CTRLZ)
    {                           /*scan code for ESC  */
      ...
      if (rms_status == RMS$_RNF)
        warn_user ("$FIND %s %s %u", strerror (EVMSERR, rms_status),
                   filterfn (__FILE__), __LINE__);
      else
        {
