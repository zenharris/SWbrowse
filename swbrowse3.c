     ...
     struct filestruct *filedef;
{
  ...
  if (!rmstest (rms_status, 2, RMS$_NORMAL, RMS$_OK_RLK))
    {
      warn_user ("GET failed - rmsret: %u %s %u", rms_status,
                 filterfn (__FILE__), __LINE__);
      return;
    }

  spawn_scroll_window (&ServFile, 10, 68);
  HeadLine ("Service Records For %s (%s)", centre,
            xunpad (fileio.record.name, SIZE_NAME),
            filterfn (ServFile.filename));
...
*/
      ...
      HeadLine ("SW Database Browser v%s - Logged in as %s", centre, VERSION,
                user_name);
/*                                                                   
      rms_status = 98728;
...
*/
      ...
      HeadLine ("Client Records File (%s)", centre,
                filterfn (PrimFile.filename));
