...

{
   ...
   if (!rmstest(rms_status,4,RMS$_NORMAL,RMS$_RNF,RMS$_OK_ALK,RMS$_OK_RLK)){ 
      warn_user("$GET %s %s %s %u",filterfn(master_file.filename),rmslookup(rms_status),filterfn(__FILE__),__LINE__);
      return;
   } 
   if (rms_status == RMS$_RNF) {
      ...
      if (!rmstest(rms_status,1,RMS$_NORMAL))
         warn_user("$PUT %s %s %s %u",filterfn(master_file.filename),rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
   }
}
...
void write_master_record()
{
   ...
   if (!rmstest(rms_status,1,RMS$_NORMAL)) 
      warn_user("$PUT %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
...
void edit_master_record(struct scrllst_msg_rec *msg,struct filestruct *filedef)
{
       ...
       if (scan != SMG$K_TRM_CTRLZ) {
          if (scan == SMG$K_TRM_PF1) {
             ...
             if(strcmp(user_name,priv_user) != 0) {
                warn_user("Not Authorised to Edit Master %s %u",filterfn(__FILE__),__LINE__);
                close_window();      
                return;
             }
             ...
             if (rmstest(rms_status,1,RMS$_RLK)){
                ...
                scan = enter_screen(master_entry_screen,MASSCRNLEN,&edited,nobreakout,NULL,&strtfld);
          }
       }
    }
    if (edited) {
