...
static void slct_invent(struct linedt_msg_rec *msg,struct filestruct *filedef) {
   quote_select_inventory_item(&quotesed_buff,filedef);
}


static void slct_kit (struct linedt_msg_rec *msg,struct filestruct *filedef){
...
int quotesed_totals(int deleting, int disc)
{
   ...
   struct quote_linerec *crnt_line = quote_entry_screen[TEXT_LINE].field,*del_test_line;
   ...
   read_master_record();
	...
	memset(taxable,0,sizeof(taxable));
      ...
      while (crnt_line->last != NULL) crnt_line = crnt_line->last;
      for (;crnt_line != NULL;crnt_line = crnt_line->next){
         if (del_test_line == crnt_line && !disc)  {
			...
			if (quotesed_buff.tax_code != 'X') {
				...
				taxable[quotesed_buff.tax_code - 'A'].w += quotesed_buff.wholesale;
			}
#else
#ifdef TAX_OPTION2
			if (quotesed_buff.tax_code != 'X') {
				taxable[quotesed_buff.tax_code - 'A'].r += quotesed_buff.wholesale;
				taxable[quotesed_buff.tax_code - 'A'].w += quotesed_buff.wholesale;
			}
#else
#ifdef TAX_OPTION3
...
#else
			if (quotesed_buff.tax_code != 'X') {
				taxable[quotesed_buff.tax_code - 'A'].r += quotesed_buff.cost_price;
				taxable[quotesed_buff.tax_code - 'A'].w += quotesed_buff.cost_price;
			}
#endif
#endif
#endif
			...
			if (crnt_line->storage.tax_code != 'X') {
				taxable[crnt_line->storage.tax_code - 'A'].r +=crnt_line->storage.retail;
				taxable[crnt_line->storage.tax_code - 'A'].w +=crnt_line->storage.wholesale;
			}
#else
#ifdef TAX_OPTION2
			if (crnt_line->storage.tax_code != 'X') {
				taxable[crnt_line->storage.tax_code - 'A'].r +=crnt_line->storage.wholesale;
				taxable[crnt_line->storage.tax_code - 'A'].w +=crnt_line->storage.wholesale;
			}
#else
#ifdef TAX_OPTION3
			if (crnt_line->storage.tax_code != 'X') {
				taxable[crnt_line->storage.tax_code - 'A'].r +=crnt_line->storage.retail;
				taxable[crnt_line->storage.tax_code - 'A'].w +=crnt_line->storage.retail;
			}
#else
			if (crnt_line->storage.tax_code != 'X') {
				taxable[crnt_line->storage.tax_code - 'A'].r +=crnt_line->storage.cost_price;
		 		taxable[crnt_line->storage.tax_code - 'A'].w +=crnt_line->storage.cost_price;
			}
#endif
#endif
#endif
      ...
      if (deleting && !(del_test_line->last == NULL && del_test_line->next == NULL)) {
         retail -= quotesed_buff.retail;
         wholesale -= quotesed_buff.wholesale;
         cost -= quotesed_buff.cost_price;
   ...
   sprintf(outbuff,"%55s%10.2f %10.2f","Tax.",(test == 'Y')?rtax:0.0,(test=='A'||test=='W'||test=='Y')?wtax:rtax);
   exclreol(18,2,0);
   exscrprn (outbuff,18,2,WHITE);
   sprintf(outbuff,"%55s%10.2f %10.2f","Totals.",(test=='A'||test=='X')?cost:(test=='Y')?retail:0.0,
...
/* no markup on non inventory items */
    ...
    if (quotesed_buff.kit) {
       ...
       if(rms_status == RMS$_RNF) warn_user("Kit %s Not Found",quotesed_buff.kit_part_code);
       else {
          memcpy(&kit_buff,kit_file.fileio,sizeof(kit_buff));
          unpad(kit_buff.kit_code,PARTKEYLEN);
          varl =(struct kit_part_rec *) (kit_file.fileio + (iter = sizeof(kit_buff)));
    
          for (;iter < dtr->reclen;varl++,iter+= sizeof (struct kit_part_rec)) {
             if (varl->part_code[0] == 0) continue;
            ...
            if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_OK_RLK,RMS$_RNF)) 
             ...
             if(rms_status != RMS$_RNF)
                ...
                warn_user("In Kit %s Part %s Not Found",kit_buff.kit_code,varl->part_code);
     ...
     if (strcmp(quotesed_buff.kit_part_code,"NONINV") != 0 &&
       ...
       if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_OK_RLK,RMS$_RNF)) 
          warn_user("$GET %s %s %s %s %u",filterfn(inventory_file.filename),
                      quotesed_buff.kit_part_code,
    ...
    if (strcmp(quotesed_buff.kit_part_code,"NONINV") == 0) {
       ...
       if (!quotesed_buff.kit || kit_buff.per_foot == 0.0) {
          ...
          quotesed_buff.wholesale = cost + (cost * (master_buff.wholesale_markup/100));
          quotesed_buff.retail = cost + (cost * (master_buff.retail_markup/100));
       }
    }
...
int recalc_costs(msg,filedef)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
{
   struct quote_linerec *crnt_line = filedef->entry_screen[filedef->entry_comment].field,*top;
   unsigned int iter,status,maxtext;
   struct quote_key0_rec quote_key;
   char *block;

   if(filedef->CurrentPtr == NULL) return(FALSE);
...
/*   filedef->rab.rab$l_ubf = (char *)filedef->fileio;
   ...
   for (crnt_line = top;crnt_line != NULL && iter < maxtext;crnt_line = crnt_line->next) {
   ...
   if (!rmstest(rms_status,3,RMS$_NORMAL,RMS$_DUP,RMS$_OK_DUP)) {
       warn_user("PUT/UPD write fail- rms retcode: %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
       return(FALSE);
   }  
...
void subtot(struct quote_part_rec *crnt_line)
{
...
void non_inventory_item(struct quote_part_rec *crnt_line)
{
...
void non_inventory_stub(struct quote_part_rec *crnt_line)
{
...
void inventory_search_2 (int inchr,int scode,int *pos_chngd)
{
...
int quote_select_inventory_item(struct quote_part_rec *crnt_line,struct filestruct *filedef)
{
   char heading[80];
   int ret,status;
...
void kit_search_2 (int inchr,int scode,int *pos_chngd)
{
...
int select_kit(struct quote_part_rec *crnt_line,struct filestruct *filedef)
{
   int ret,status;
   unsigned long rec_num;
...
*/
         ...
         if (!rmstest(rms_status,5,RMS$_NORMAL,RMS$_RNF,RMS$_RLK,RMS$_RNF,RMS$_OK_RLK)) 
            warn_user("$GET %s %s %s %u",filterfn(kit_file.filename),rmslookup(rms_status),filterfn(__FILE__),__LINE__); 
         else
            if (rmstest(rms_status,1,RMS$_RLK)) {
               /* if (rms_status == RMS$_RLK) warn_user ("%s %s %u",strerror (EVMSERR, rms_status),filterfn(__FILE__),__LINE__);
               else */ warn_user("%s %s %u",strerror (EVMSERR, rms_status),filterfn(__FILE__),__LINE__); 
   ...
   if (rms_status != RMS$_RNF && strncmp(code,dtr->kit_code,strlen(code))==0){
   ...
   if (rms_status != RMS$_RNF && strncmp(code,inventory_buff.part_code,strlen(code))==0){
         strncpy(quotesed_buff.kit_part_code,inventory_buff.part_code,PARTKEYLEN); 
         unpad(quotesed_buff.kit_part_code,PARTKEYLEN);
         strcpy(quotesed_buff.description,inventory_buff.description);
         strcpy(quotesed_buff.units,inventory_buff.units);
         quotesed_buff.kit = FALSE;
   ...
   warn_user("$GETEQ %s %s %u",rmslookup(rms_status),filterfn(__FILE__),__LINE__);
   return(TRUE);
}

extern struct filestruct *G_filedef;

int quotesed(edtstr,row,col,lnth,attr,pos,control)
char *edtstr;
...
/* struct filestruct *filedef; */
{
   ...
   struct line_rec *curs;
	...
	switch (master_master.d_flag_w) {
 		case 'W' :	quote_line = quote_line_whole;
						linlen = LINEDLEN;
						break;
 		case 'R' :	quote_line = quote_line_retail;
						linlen = LINEDLEN;
						break;
 		case 'X'  :	quote_line = quote_line_all_retail;
						linlen = LINEDLEN;
						break;
 		case 'Y'  :	quote_line = quote_line_retail_whole;
						linlen = LINEDLEN;
						break;
 		default  :	quote_line = quote_line_all;
						linlen = LINEDLEN;
						break;
	}
   ...
   if (*control & INIT_ONLY) {
      for (i = 0; i < linlen;i++) quote_line[i].row = row;
      init_screen(quote_line,linlen);
      return(TRUE);
   } else if (*control & ALLOCATE) {
      (*(struct quote_linerec **)edtstr) = default_ptr = lmalloc(sizeof(quotesed_buff));
      ...
      return(TRUE);
       ...
       return ( memcmp (&comp_buff,&quotesed_buff,sizeof(quotesed_buff)) != 0);
   }
   for (i = 0; i < linlen;i++) quote_line[i].row = row;

   init_screen(quote_line,linlen);
   ret = enter_screen(quote_line,linlen,&edited,breakout,&quote_file,&strtfld);
   switch (ret) {
      case SMG$K_TRM_CTRLD: quotesed_totals(TRUE,FALSE);   
                            break;
      case SMG$K_TRM_RIGHT:
      case SMG$K_TRM_CTRLZ:
      case SMG$K_TRM_CR:
      case SMG$K_TRM_HT: strtfld = 0;
                         break;
      case SMG$K_TRM_UP: curs = quote_file.entry_screen[quote_file.entry_comment].field;
                         if(curs->last == NULL) strtfld = 0;
                         break;
