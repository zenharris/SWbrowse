...
void kit_search(msg,filedef,frstchr)
struct linedt_msg_rec *msg;
struct filestruct *filedef;
char frstchr;
{
   ...
   if (wch != SMG$K_TRM_CTRLZ){ 
      ...
      if (rms_status == RMS$_RNF) 
         warn_user("$FIND RMS$_RNF Record Not Found %s %u",filterfn(__FILE__),__LINE__);
      else {
...
void kit_pad_record(void)
{
   struct kit_rec *dtr = (struct kit_rec *)kit_file.fileio;
   pad(dtr->kit_code,PARTKEYLEN);
}

void kit_unpad_record(void)
{
   struct kit_rec *dtr = (struct kit_rec *)kit_file.fileio;
...
void disassemble(block,scrl_consts,v_lnth,v_ptr,reclen)
char *block;
...
int assemble(block,scrl_consts,v_lnth,v_ptr)
char **block;
struct filestruct *scrl_consts;
int v_lnth;
struct line_rec **v_ptr;
...
int kit_unload_editor_buffer(curs,filedef)
...
struct filestruct *filedef;
{
   ...
   lines = assemble(&filedef->fileio,filedef,sizeof(struct kit_part_rec),&curs);
...
void kit_load_editor_buffer (ptr,filedef)
struct line_rec **ptr;
struct filestruct *filedef;
{
   struct kit_rec *dtr = (struct kit_rec *)filedef->fileio;

   disassemble(filedef->fileio,filedef,sizeof(struct kit_part_rec),ptr,dtr->reclen);
...
void fork_kit_file(struct scrllst_msg_rec *msg)
{
